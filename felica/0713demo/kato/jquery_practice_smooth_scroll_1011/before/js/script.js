...
//  ***** スムーズスクロールの -- offset().top --  設定 *****  
    // navタグのaタグの場所を取得 --> 'nav a' --> nav_menuの変数に

        // nav_menuをクリックしたとき、個々のaタグの属性、hrefの属性値を取得 --> 変数、const hrefに代入 
    
        // 取得した属性値の表示されたトップの位置をピクセル単位で取得 --> 変数、const targetに代入 --> $(href).offset().top
        ...
        nav_menu.on('touchstart click',function(){
            ...
            console.log(href);
            // const target = $(href).offset().top - 50;
            // const target = $(href).offset().top;
            let target = $(href).offset().top;
            console.log(target);

            // クリックした際のブラウザの幅を取得し、条件分岐する。
